// Test específico para el hash que estaba fallando
const IPFS_GATEWAYS = [
...
async function testSpecificHash() {
  // El hash específico que estaba fallando
  const problemHash = 'QmNLei78zWmzUdbeRB3CiUfAizWUrbeeZh5K1rhAQKCh51';
  
  console.log(`🔍 Probando hash específico: ${problemHash.slice(0, 15)}...`);
  console.log('Este es el hash que estaba causando errores de timeout\n');
  
  const results = [];
  
  for (let i = 0; i < IPFS_GATEWAYS.length; i++) {
    const gateway = IPFS_GATEWAYS[i];
    const url = gateway + problemHash;
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Timeout después de 10000ms')), 10000);
      });
